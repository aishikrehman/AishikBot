...
(function () {
    ...
    JWB.imports = {
        "JWB.css": "//test.wikipedia.org/w/index.php?title=User:Aishik_Rehman/JWB.css&action=raw&ctype=text/css",
...
(function () {
    ...
    // mw.loader.load( 'https://cdn.jsdelivr.net/npm/bootstrap@5.2.1/dist/js/bootstrap.bundle.min.js' );
    // mw.loader.load( 'https://cdn.jsdelivr.net/npm/bootstrap@5.2.1/dist/css/bootstrap.min.css', 'text/css' );
    $.getScript(JWB.imports["i18n.js"], function () {
        ...
        if (langs.length) {
            $.when.apply($, langs.map(url => $.getScript(url))).done(function (r) {
                if (JWB.allowed === true && JWB.messages.length == langs + 1) {
                    // if there are two languages to load, wait for them both.
    ...
    new mw.Api().get({
        action: "query",
        titles: "User:Aishik Rehman/JWB/CheckPageJSON",
        prop: "info|revisions",
        ...
        if (response.query.pageids[0] !== "-1") {
            ...
            if (JWB.sysop) {
                // Check and inform admins if their checkpage is the unsupported format.
                new mw.Api().get({
                    action: "query",
                    titles: "User:Aishik Rehman/JWB/CheckPage",
                    prop: "info",
                    indexpageids: true
                    ...
                    if (q.pageids[0] != "-1" && !q.pages[q.pageids[0]].hasOwnProperty("redirect")) {
                        ...
                        prompt("Warning: The AWB checkpage found at User:Aishik Rehman/JWB/CheckPage is no longer supported.\n" + "Please convert this checkpage to a JSON checkpage. See the URL below for more information.\n" + 'After creating the JSON checkpage, you can use "Special:ChangeContentModel" to change the content model to JSON.', "https://en.wikipedia.org/wiki/Wikipedia:AutoWikiBrowser/CheckPage_format");
                    }
                });
            }
        }

        JWB.bot = groups.indexOf("user") !== -1 && (users === false || bots.includes(JWB.username)); // Temporary global debugging variables

        JWB.debug = [groups.indexOf("user"), users === false, bots && bots.indexOf(JWB.username)];

        if (JWB.username === "Aishik_Rehman" && response.query.userinfo.id === 13299994) {
            //TEMP: Dev full access to entire interface.
            JWB.bot = true;
            users.push("Aishik_Rehman");
        }
...
JWB.api.diff = function (callback) {
    ...
    JWB.api.call(data, function (response) {
        ...
        if (diff === "") {
            ...
            diff = '<table class="diff">' + "<colgroup>" + '<col class="diff-marker">' + '<col class="diff-content">' + '<col class="diff-marker">' + '<col class="diff-content">' + "</colgroup>" + "<tbody>" + diff + "</tbody></table>";
        }
        ...
        $("table.diff tr").each(function () {
            //add data-line attribute to every line, relative to the previous one. Used for click event.
...
JWB.api.get = function (pagename) {
    ...
    let cgns = JWB.ns[14]["*"];
    let skipcg = $("#skipCategories").val(); // prepend Category: before all categories and turn CSV(,) into CSV(|).
    ...
    JWB.api.call(data, function (response) {
        if (response.query.userinfo.hasOwnProperty("messages")) {
            ...
            JWB.status('<span style="color:red;font-weight:bold;">' + JWB.msg("status-newmsg", '<a href="' + view + '" target="_blank">' + JWB.msg("status-talklink") + "</a>", '<a href="' + viewNew + '" target="_blank">' + JWB.msg("status-difflink") + "</a>") + "</span>", true);
        ...
        if (!JWB.fn.allowBots(JWB.page.content, JWB.username) || !JWB.fn.allowBots(JWB.page.content)) {
            ...
            $("#exists-no").prop("checked") && !JWB.page.exists || $("#exists-yes").prop("checked") && JWB.page.exists || redirect === "skip" && response.query.redirects // variable  redirect  is defined outside this callback function.
        ) {
            // simple skip rules
        ...
        if (containRegex) {
            ...
            if (skipContains && JWB.page.content.includes(skipContains) || skipNotContains && !JWB.page.content.includes(skipNotContains)) {
                console.log("skipped page before replaces");
                return JWB.next();
            }
        ...
        JWB.replace(JWB.page.content, function (newContent) {
            ...
            if ($("#skipNoChange").prop("checked") && JWB.page.content === newContent) {
...
JWB.api.protect = function () {
    ...
    if (!JWB.page.exists) protstring = "create=" + editprot;
    if (JWB.page.protections.includes("upload")) protstring += "|upload=" + uploadprot;
    JWB.api.call({
        action: "protect",
        title: JWB.page.name,
        token: JWB.page.token,
...
JWB.api.watch = function () {
    ...
    JWB.api.call(data, function (response) {
        JWB.status('<span style="color:green;">' + JWB.msg("status-watch-" + (JWB.page.watched ? "removed" : "added"), "'" + JWB.page.name + "'") + "</span>", true);
...
JWB.pl.getList = function (abbrs, lists, data) {
    ...
    JWB.api.call(data, function (response) {
        ...
        if (cont && JWB.pl.iterations <= maxiterate) {
            ...
            if (response.query) {
                //compatibility with the code I wrote for the old query-continue. TODO: make this unnecessary?
        ...
        //on error, simply reset and let the user work with what he has
        JWB.status("done", true);
        JWB.pl.stop();
        JWB.pl.done = true;
    });
...
JWB.pl.generate = function () {
    ...
    $fields.each(function () {
        ...
        if (list === "linksto") {
            //Special case since this fieldset features 3 merged lists in 1 fieldset
    ...
    if (abbrs.length || data.ask) JWB.pl.getList(abbrs, lists, data); else JWB.pl.stop();
};
/***** Setup functions *****/
...
JWB.setup.apply = function (name) {
    ...
    $(".useRegex, #containRegex," + "#pagelistPopup legend input," + "#enableRETF").trigger("change"); //reset disabled inputs
};
...
JWB.setup.download = function () {
    ...
    if (HTMLAnchorElement.prototype.hasOwnProperty("download")) {
        //use download attribute when possible, for its ability to specify a filename
        elem.href = url;
        elem.click();
        setTimeout(function () {
        ...
        //fallback to iframes for browsers with no support for download="" attributes
...
JWB.setup.load = function () {
    ...
    var titles = oldtitle; // if the old title isn't JWB-settings.json, also query the new title.
    ...
    JWB.api.call({
        ...
        var edittoken = response.query.tokens.csrftoken; // determine correct page to get settings from
...
JWB.setup.moveNew = function (from, to, token) {
    new mw.Api().post({
        ...
        noredirect: true,
        // if possible, suppress redirects; the old page will no longer be needed if the new page exists.
        movesubpages: true,
        // if any
        movetalk: true,
        // if any
...
JWB.log = function (action, page, info) {
    ...
    $("#actionlog tbody").append("<tr>" + "<td>" + (JWB.fn.pad0(d.getHours()) + ":" + JWB.fn.pad0(d.getMinutes()) + ":" + JWB.fn.pad0(d.getSeconds())) + "</td>" + "<th>" + action + "</th>" + '<td><a target="_blank" href="/wiki/' + pagee + '" title="' + page + '">' + page + "</a>" + extraInfo + "</td>" + "</tr>").parents(".JWBtabc").scrollTop($("#actionlog tbody").parents(".JWBtabc")[0].scrollHeight);
...
JWB.stop = function () {
    ...
    $("#stopbutton," + ".editbutton," + "#watchNow," + '.JWBtabc[data-tab="2"] .editbutton,' + "#watchNow" + '.JWBtabc[data-tab="4"] button,' + "#skipRETF").prop("disabled", true);
    $("#startbutton, #articleList," + '.JWBtabc[data-tab="1"] button,' + "#replacesPopup button," + "#replacesPopup input," + ".JWBtabc input, select").prop("disabled", false);
    $("#resultWindow").html("");
    $("#editBoxArea").val("");
    $("#currentpage").html(JWB.msg("editbox-currentpage", " ", " "));
    JWB.pl.done = true;
...
JWB.start = function () {
    ...
    if (JWB.list.length === 0 || JWB.list.length === 1 && !JWB.list[0]) {
        ...
        if (!JWB.bot || !$("#autosave").prop("checked")) {
            ...
            $("#minorEdit, #summary, #watchPage").prop("disabled", false);
        }
...
JWB.worker.queue = []; // Load function required to properly load the worker, since directly using `new Worker(url)` for cross-origin URLs does not work even with CORS/CSP rules all allowing it.
...
JWB.worker.load = function (callback) {
    ...
    $.getScript(JWB.imports["worker.js"], function () {
        ...
        JWB.worker.functions = JWB.worker.function(); // the loaded script just defined JWB.worker.function; convert it to a blob url
        // Based on https://stackoverflow.com/a/33432215/1256925

        if (JWB.worker.supported) try {
            let blob = new Blob(["(" + JWB.worker.function.toString() + ")()"], {
...
JWB.worker.do = function (msg, callback) {
    if (JWB.worker.isWorking()) {
        JWB.worker.queue.push({
            msg: msg,
            callback: callback
        });
    } else {
        var timelimit = parseInt($("#timelimit").val()) || 3000;
        JWB.worker.callback = callback; // Expand "JWB.string" into JWB['string']; to allow the string to be loaded at execution time instead of queue time.
...
JWB.fn.allowBots = function (text, user = "JWB") {
    ...
    if (new RegExp("\\{\\{\\s*bots\\s*\\|\\s*deny\\s*=\\s*([^}]*,\\s*)*" + usr + "\\s*(?=[,\\}])[^}]*\\s*\\}\\}", "i").test(text)) return false; else return new RegExp("\\{\\{\\s*((?!nobots)|bots(\\s*\\|\\s*allow\\s*=\\s*((?!none)|([^}]*,\\s*)*" + usr + "\\s*(?=[,\\}])[^}]*|all))?|bots\\s*\\|\\s*deny\\s*=\\s*(?!all)[^}]*|bots\\s*\\|\\s*optout=(?!all)[^}]*)\\s*\\}\\}", "i").test(text);
...
JWB.fn.blink = function (el, t) {
    t = t ? t : 500;
    $(el).prop("disabled", true).children().animate({
        opacity: "0.1"
    }, t - 100).animate({
        opacity: "1"
    }, t).animate({
...
JWB.fn.scrollSelection = function (el, index) {
    //function to fix scrolling to selection - doesn't do that automatically.
...
JWB.init = function () {
    ...
    var findreplace = '<div class="replaces">' + '<label style="display:block;">' + JWB.msg("label-replace") + ' <input type="text" class="replaceText"/></label>' + '<label style="display:block;">' + JWB.msg("label-rwith") + ' <input type="text" class="replaceWith"/></label>' + '<div class="regexswitch">' + '<label><input type="checkbox" class="useRegex"> ' + JWB.msg("label-useregex") + "</label>" + '<a class="re101" href="http://regex101.com/#javascript" target="_blank">?</a>' + '<label class="divisor" title="' + JWB.msg("tip-regex-flags") + '" style="display:none;">' + JWB.msg("label-regex-flags") + ' <input type="text" class="regexFlags" value="g"/>' + //default: global replacement
        "</label>" + "<br/>" + "</div>" + '<label title="' + JWB.msg("tip-ignore-comment") + '">' + '<input type="checkbox" class="ignoreNowiki"> ' + JWB.msg("label-ignore-comment") + "</label>" + "</div>";
    ...
    $("body").html('<article id="resultWindow"></article>' + '<main id="inputsWindow">' + '<div id="inputsBox">' + '<aside id="articleBox">' + "<b>" + JWB.msg("pagelist-caption") + "</b>" + '<textarea id="articleList"></textarea>' + "</aside>" + '<section id="tabs">' + '<nav class="tabholder">' + '<span class="JWBtab" data-tab="1">' + JWB.msg("tab-setup") + "</span> " + '<span class="JWBtab active" data-tab="2">' + JWB.msg("tab-editing") + "</span> " + '<span class="JWBtab" data-tab="3">' + JWB.msg("tab-skip") + "</span> " + '<span class="JWBtab" data-tab="4">' + JWB.msg("tab-other") + "</span>" + ' <span class="JWBtab log" data-tab="5">' + JWB.msg("tab-log") + "</span> " + "</nav>" + '<section class="JWBtabc" data-tab="1"></section>' + '<section class="JWBtabc active" data-tab="2"></section>' + '<section class="JWBtabc" data-tab="3"></section>' + '<section class="JWBtabc" data-tab="4"></section>' + '<section class="JWBtabc log" data-tab="5"></section>' + '<footer id="status">done</footer>' + "</section>" + '<aside id="editBox">' + "<b>" + JWB.msg("editbox-caption") + ' - <span id="currentpage">' + JWB.msg("editbox-currentpage", " ", " ") + "</span></b>" + '<textarea id="editBoxArea"></textarea>' + "</aside>" + "</div>" + "</main>" + '<footer id="stats">' + JWB.msg("stat-pages") + ' <span id="totPages">0</span>;&emsp;' + JWB.msg("stat-save") + ' <span id="pagesSaved">0</span>;&emsp;' + JWB.msg("stat-null") + ' <span id="nullEdits">0</span>;&emsp;' + JWB.msg("stat-skip") + ' <span id="pagesSkipped">0</span>;&emsp;' + JWB.msg("stat-other") + ' <span id="otherActions">0</span>;&emsp;' + "</footer>" + '<div id="overlay" style="display:none;"></div>' + '<section class="JWBpopup" id="replacesPopup" style="display:none;">' + '<button id="moreReplaces">' + JWB.msg("button-more-fields") + "</button>" + "<br>" + findreplace + "</section>" + '<section class="JWBpopup" id="pagelistPopup" style="display:none;">' + '<form action="#" id="pl-form"></form>' + "</section>");
    $('.JWBtabc[data-tab="1"]').html('<fieldset id="pagelist">' + "<legend>" + JWB.msg("label-pagelist") + "</legend>" + '<button id="removeDupes">' + JWB.msg("button-remove-dupes") + "</button> " + '<button id="sortArticles">' + JWB.msg("button-sort") + "</button>" + "<br>" + '<label title="' + JWB.msg("tip-preparse") + '">' + '<input type="checkbox" id="preparse"> ' + JWB.msg("preparse") + "</label>" + '<span class="divisor"></span>' + '<button id="preparse-reset" title="' + JWB.msg("tip-preparse-reset") + '">' + JWB.msg("preparse-reset") + "</button>" + "<br>" + '<button id="pagelistButton">' + JWB.msg("pagelist-generate") + "</button>" + "</fieldset>" + '<fieldset id="settings">' + "<legend>" + JWB.msg("label-settings") + "</legend>" + '<button id="saveAs" title="' + JWB.msg("tip-store-setup") + '">' + JWB.msg("store-setup") + "</button>" + "<br>" + "<label>" + JWB.msg("load-settings") + " " + '<select id="loadSettings">' + '<option value="default" selected>default</option>' + '<option value="_blank">' + JWB.msg("blank-setup") + "</option>" + "</select>" + "</label>" + '<span class="divisor"></span>' + '<button id="deleteSetup" title="' + JWB.msg("tip-delete-setup") + '">' + JWB.msg("delete-setup") + "</button>" + "<hr>" + '<button id="saveToWiki">' + JWB.msg("save-setup") + "</button>" + '<span class="divisor"></span>' + '<button id="download">' + JWB.msg("download-setup") + "</button>" + "<hr>" + '<label class="button" id="importLabel" title="' + JWB.msg("tip-import-setup") + '">' + '<input type="file" id="import" accept=".json">' + JWB.msg("import-setup") + "</label>" + '<span class="divisor"></span>' + '<button id="updateSetups" title="' + JWB.msg("tip-update-setup", JWB.settingspage) + '">' + JWB.msg("update-setup") + "</button>" + '<div id="downloads">' + '<a download="JWB-settings.json" target="_blank" id="download-anchor"></a>' + '<iframe id="download-iframe"></iframe>' + "</div>" + "</fieldset>" + '<fieldset id="limits">' + "<legend>" + JWB.msg("label-limits") + "</legend>" + '<label class="timelimit-label" title="' + JWB.msg("tip-time-limit") + '">' + JWB.msg("time-limit") + '<input type="number" id="timelimit" value="3000" min="0">' + "</label>" + '<label title="' + JWB.msg("tip-diff-size-limit") + '">' + JWB.msg("diff-size-limit") + '<input type="number" id="sizelimit" value="0" min="0">' + "</label>" + "</fieldset>");
    $('.JWBtabc[data-tab="2"]').html('<label class="minorEdit"><input type="checkbox" id="minorEdit" checked> ' + JWB.msg("minor-edit") + "</label>" + '<label class="editSummary">' + JWB.msg("edit-summary") + '<br/> <input class="fullwidth" type="text" id="summary" maxlength="500"></label>' + '<select id="watchPage">' + '<option value="watch">' + JWB.msg("watch-watch") + "</option>" + '<option value="unwatch">' + JWB.msg("watch-unwatch") + "</option>" + '<option value="nochange" selected>' + JWB.msg("watch-nochange") + "</option>" + '<option value="preferences">' + JWB.msg("watch-preferences") + "</option>" + "</select>" + '<span class="divisor"></span>' + '<button id="watchNow" disabled accesskey="w">' + JWB.msg("watch-add") + "</button>" + "<br>" + (JWB.bot ? '<label><input type="checkbox" id="autosave"> ' + JWB.msg("auto-save") + "</label>" + '<label title="' + JWB.msg("tip-save-interval") + '" class="divisor">' + JWB.msg("save-interval", '<input type="number" min="0" value="0" style="width:50px" id="throttle" disabled>') + "</label>" + "<br>" : "") + '<span id="startstop">' + '<button id="startbutton" accesskey="a">' + JWB.msg("editbutton-start") + "</button>" + "<br>" + '<button id="stopbutton" disabled accesskey="q">' + JWB.msg("editbutton-stop") + "</button> " + "</span>" + '<button class="editbutton" id="skipButton" disabled accesskey="n">' + JWB.msg("editbutton-skip") + "</button>" + '<button class="editbutton" id="submitButton" disabled accesskey="s">' + JWB.msg("editbutton-save") + "</button>" + "<br>" + '<button class="editbutton" id="previewButton" disabled accesskey="p">' + JWB.msg("editbutton-preview") + "</button>" + '<button class="editbutton" id="diffButton" disabled accesskey="d">' + JWB.msg("editbutton-diff") + "</button>" + '<button id="replacesButton">' + JWB.msg("button-open-popup") + "</button>" + findreplace + "<hr>" + '<label><input type="checkbox" id="enableRETF"> ' + JWB.msg("label-enable-RETF", '<a href="/wiki/Project:AutoWikiBrowser/Typos" target="_blank">' + JWB.msg("label-RETF") + "</a>") + "</label>" + ' <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/2/2a/Gnome-view-refresh.svg/20px-Gnome-view-refresh.svg.png"' + 'id="refreshRETF" title="' + JWB.msg("tip-refresh-RETF") + '">' + "<br/>" + '<button id="skipRETF" title="' + JWB.msg("tip-skip-RETF") + '" disabled>' + JWB.msg("skip-RETF") + "</button>");
    $('.JWBtabc[data-tab="3"]').html("<fieldset>" + "<legend>" + JWB.msg("label-redirects") + "</legend>" + '<label title="' + JWB.msg("tip-redirects-follow") + '">' + '<input type="radio" class="redirects" value="follow" name="redir" id="redir-follow"> ' + JWB.msg("redirects-follow") + " " + "</label>" + '<label title="' + JWB.msg("tip-redirects-skip") + '">' + '<input type="radio" class="redirects" value="skip" name="redir" id="redir-skip"> ' + JWB.msg("redirects-skip") + " " + "</label>" + '<label title="' + JWB.msg("tip-redirects-edit") + '">' + '<input type="radio" class="redirects" value="edit" name="redir" id="redir-edit" checked> ' + JWB.msg("redirects-edit") + "" + "</label>" + "</fieldset>" + "<fieldset>" + "<legend>" + JWB.msg("label-skip-when") + "</legend>" + '<label><input type="checkbox" id="skipNoChange"> ' + JWB.msg("skip-no-change") + "</label>" + "<br>" + '<label><input type="radio" id="exists-yes" name="exists" value="yes"> ' + JWB.msg("skip-exists-yes") + "</label>" + '<label><input type="radio" id="exists-no" name="exists" value="no" checked> ' + JWB.msg("skip-exists-no") + "</label>" + '<label><input type="radio" id="exists-neither" name="exists" value="neither">' + JWB.msg("skip-exists-neither") + "</label>" + '<br><label><input type="checkbox" id="skipAfterAction" checked> ' + JWB.msg("skip-after-action") + "</label>" + "<hr/>" + "<label>" + JWB.msg("skip-contains") + ' <input class="fullwidth" type="text" id="skipContains"></label>' + "<label>" + JWB.msg("skip-not-contains") + ' <input class="fullwidth" type="text" id="skipNotContains"></label>' + '<div class="regexswitch">' + '<label><input type="checkbox" id="containRegex"> ' + JWB.msg("label-useregex") + "</label>" + '<a class="re101" href="http://regex101.com/#javascript" target="_blank">?</a>' + '<label class="divisor" title="' + JWB.msg("tip-regex-flags") + '" style="display:none;">' + JWB.msg("label-regex-flags") + ' <input type="text" id="containFlags"/>' + "</label>" + "</div>" + "<hr/>" + '<label title="' + JWB.msg("skip-cg-prefix") + '">' + JWB.msg("skip-category") + ' <input class="fullwidth" type="text" id="skipCategories"></label>' + "</fieldset>");
    $('.JWBtabc[data-tab="4"]').html("<fieldset>" + "<legend>" + JWB.msg("move-header") + "</legend>" + '<label><input type="checkbox" id="suppressRedir"> ' + JWB.msg("move-redir-suppress") + "</label>" + "<br>" + JWB.msg("move-also") + " " + '<label><input type="checkbox" id="movetalk"> ' + JWB.msg("move-talk-page") + "</label> " + '<label><input type="checkbox" id="movesubpage"> ' + JWB.msg("move-subpage") + "</label>" + "<br>" + "<label>" + JWB.msg("move-new-name") + ' <input type="text" id="moveTo"></label>' + "</fieldset>" + "<fieldset>" + "<legend>" + JWB.msg("protect-header") + "</legend>" + JWB.msg("protect-edit") + ' <select id="editProt">' + '<option value="all" selected>' + JWB.msg("protect-none") + "</option>" + '<option value="autoconfirmed">' + JWB.msg("protect-autoconf") + "</option>" + '<option value="sysop">' + JWB.msg("protect-sysop") + "</option>" + "</select> " + "<br>" + JWB.msg("protect-move") + ' <select id="moveProt">' + '<option value="" selected>(' + JWB.msg("protect-like-edit") + ")</option>" + '<option value="all">' + JWB.msg("protect-none") + "</option>" + '<option value="autoconfirmed">' + JWB.msg("protect-autoconf") + "</option>" + '<option value="sysop">' + JWB.msg("protect-sysop") + "</option>" + "</select> " + "<br>" + JWB.msg("protect-upload") + ' <select id="uploadProt">' + '<option value="" selected>(' + JWB.msg("protect-like-edit") + ")</option>" + '<option value="all">' + JWB.msg("protect-none") + "</option>" + '<option value="autoconfirmed">' + JWB.msg("protect-autoconf") + "</option>" + '<option value="sysop">' + JWB.msg("protect-sysop") + "</option>" + "</select> " + "<br>" + "<label>" + JWB.msg("protect-expiry") + ' <input type="text" id="protectExpiry"/></label>' + "</fieldset>" + '<button id="movePage" disabled accesskey="m">' + JWB.msg("editbutton-move") + "</button> " + '<button id="deletePage" disabled accesskey="x">' + JWB.msg("editbutton-delete") + "</button> " + '<button id="protectPage" disabled accesskey="z">' + JWB.msg("editbutton-protect") + "</button> " + '<button id="skipPage" disabled title="[' + JWB.tooltip + 'n]">' + JWB.msg("editbutton-skip") + "</button>");
    $('.JWBtabc[data-tab="5"]').html('<table id="actionlog"><tbody></tbody></table>');
    $("#pagelistPopup form").html('<div id="ns-filter" title="' + JWB.msg("tip-ns-select") + '">' + JWB.msg("label-ns-select") + NSList + "</div>" + "<fieldset>" + '<legend><label><input type="checkbox" id="categorymembers" name="categorymembers" value="cm"> ' + JWB.msg("legend-cm") + "</label></legend>" + '<label title="' + JWB.msg("tip-cm") + '">' + JWB.msg("label-cm") + ' <input type="text" name="cmtitle" id="cmtitle"></label>' + "<div>" + JWB.msg("cm-include") + " " + '<label><input type="checkbox" id="cmtype-page" name="cmtype" value="page" checked> ' + JWB.msg("cm-include-pages") + "</label>" + '<label><input type="checkbox" id="cmtype-subcg" name="cmtype" value="subcat" checked> ' + JWB.msg("cm-include-subcgs") + "</label>" + '<label><input type="checkbox" id="cmtype-file" name="cmtype" value="file" checked> ' + JWB.msg("cm-include-files") + "</label>" + "</div>" + "</fieldset>" + "<fieldset>" + '<legend><label><input type="checkbox" name="linksto" id="linksto"> ' + JWB.msg("legend-linksto") + "</label></legend>" + "<label>" + JWB.msg("label-linksto") + ' <input type="text" name="title" id="linksto-title"></label>' + "<div>" + JWB.msg("links-include") + " " + '<label><input type="checkbox" id="backlinks" name="backlinks" value="bl" checked> ' + JWB.msg("links-include-links") + "</label>" + '<label><input type="checkbox" id="embeddedin" name="embeddedin" value="ei"> ' + JWB.msg("links-include-templ") + "</label>" + '<label><input type="checkbox" id="imageusage" name="imageusage" value="iu"> ' + JWB.msg("links-include-files") + "</label>" + "</div>" + "<div>" + JWB.msg("links-redir") + " " + '<label><input type="radio" id="rfilter-redir" name="filterredir" value="redirects"> ' + JWB.msg("links-redir-redirs") + "</label>" + '<label><input type="radio" id="rfilter-nonredir" name="filterredir" value="nonredirects"> ' + JWB.msg("links-redir-noredirs") + "</label>" + '<label><input type="radio" id="rfilter-all" name="filterredir" value="all" checked> ' + JWB.msg("links-redir-all") + "</label>" + "</div>" + '<label title="' + JWB.msg("tip-link-redir") + '">' + '<input type="checkbox" name="redirect" value="true" checked id="linksto-redir"> ' + JWB.msg("label-link-redir") + "</label>" + "</fieldset>" + "<fieldset>" + '<legend><label><input type="checkbox" id="prefixsearch" name="prefixsearch" value="ps"> ' + JWB.msg("legend-ps") + "</label></legend>" + "<label>" + JWB.msg("label-ps") + ' <input type="text" name="pssearch" id="pssearch"></label>' + '<label title="' + JWB.msg("tip-ps-strict") + '"><input type="checkbox" name="allpages" value="ap" id="psstrict" checked> ' + JWB.msg("label-ps-strict") + "</label>" + "</fieldset>" + "<fieldset>" + '<legend><label><input type="checkbox" id="watchlistraw" name="watchlistraw" value="wr"> ' + JWB.msg("legend-wr") + "</label></legend>" + JWB.msg("label-wr") + "</fieldset>" + "<fieldset>" + '<legend><label><input type="checkbox" id="proplinks" name="links" value="pl"> ' + JWB.msg("legend-pl") + "</label></legend>" + '<label title="' + JWB.msg("tip-pl") + '">' + JWB.msg("label-pl") + ' <input type="text" id="titles" name="titles"></label>' + "</fieldset>" + "<fieldset>" + '<legend><label><input type="checkbox" id="proplinks" name="search" value="sr"> ' + JWB.msg("legend-sr") + "</label></legend>" + '<label title="' + JWB.msg("tip-sr") + "\n" + JWB.msg("placeholder-sr", "insource:", "intitle:") + '">' + JWB.msg("label-sr") + ' <input type="text" id="srsearch" name="srsearch" placeholder="' + JWB.msg("placeholder-sr", "insource:", "intitle:") + '">' + "</label>" + "</fieldset>" + '<fieldset class="listSMW">' + '<legend><label><input type="checkbox" id="smwask" name="smwask" value="smw"> ' + JWB.msg("legend-smw", JWB.msg("smw-slow")) + "</label></legend>" + '<textarea id="smwquery" name="smwquery" placeholder="' + JWB.msg("label-smw", "\n|limit=500") + '"></textarea>' + "</fieldset>" + '<button type="submit">' + JWB.msg("pagelist-generate") + "</button>");
...
$("html").attr('class', 'js-wiki-browser-dashboard');$("body").removeAttr('class');
$("body").attr('class', 'js-wiki-browser');
